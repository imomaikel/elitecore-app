 ...
 */
export const serverSelection = async ({
    action,
    interaction,
}: TServerSelection) => {
    ...
    const searchForStatus =
        action === 'stop' || action === 'restart' ? 'online' : 'offline';
    const possibleServers = statuses.filter(
        (server) => server.currentStatus === searchForStatus,
    );
    ...
    for (const status of possibleServers) {
        const serverDetails = storedServers.find(
            (server) => server.id === status.serverId,
        );
        if (!serverDetails) continue;
        const serverName = `${serverDetails.mapName} ${
            serverDetails.customName ? `(${serverDetails.customName})` : ''
        }`;
        options.push(
            new StringSelectMenuOptionBuilder()
                .setLabel(serverName)
                .setDescription(
                    `${serverDetails.gameType} ${serverDetails.gameMode}`,
                )
