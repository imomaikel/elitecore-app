...
const serverControlLog = async ({
    ...
    for (let i = 0; i < dataSize; i++) {
        const serverDetails = storedServers.find(
            (entry) =>
                entry.id ===
                (typeof serversOrId === 'number'
                    ? serversOrId
                    : serversOrId[i].serverId),
        );
        ...
        const footerText =
            restartedBy === 'auto'
                ? 'Auto restarted'
                : `${restartedBy.username} took the action`;
        const footerImage =
            restartedBy === 'auto'
                ? specialAvatar
                : restartedBy.client.user.displayAvatarURL();
