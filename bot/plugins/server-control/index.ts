...
export const updateServerControlWidget = async (): Promise<boolean> => {
    ...
    const onlineServers = data.filter(
        (entry) => entry.currentStatus === 'online',
    ).length;
    const offlineServers = data.filter(
        (entry) => entry.currentStatus === 'offline',
    ).length;
    ...
    if (showStopButton) {
        buttons.push(
            new ButtonBuilder()
                ...
                .setLabel('STOP')
                .setCustomId(`${client.user?.id}|server-stop`),
        );
        buttons.push(
            new ButtonBuilder()
    ...
    for (const server of data) {
        ...
        const isAutoRestartSign = serverDetails.autoRestart
            ? `${star} `
            : ':map: ';
